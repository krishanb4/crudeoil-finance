...
import { formatApy, formatTvl, calcDaily, formatTotalApy } from '../../helpers/format';
...
const PoolCard = ({ classes, width, tokens, pool, apys, isListView, index }) => {
  ...
  var token = tokens.find(t => t.get('token') == pool.get('token'));

  var poolApy = apys.find(t => t.get('token') == pool.get('id'));

  var rewardApy = apys.find(t => t.get('token') == pool.get('rewardAprKey'));
  ...
  const calTotalApy = () => {
    return formatTotalApy(poolApy || 0 , rewardApy || 0);
  };
  ...
  return (
    ...
    >
      {showAdditionalInfo && (
        ...
        >
          <div className={classes.showAdditionalInfoBtn} onClick={hideAdditionalInfo}>
            <Ionicon
              className={classes.showAdditionalInfoIcon}
      ...
      <CardContent
        className={classNames(classes.floatingButtonWrap, isListView ? classes.cardFlexGrow : '')}
      >
        <div
          className={classNames(
        ...
        <section className={classes.boostedTagDiv}>
          {pool.get('depositsPaused') && (
            <div className={classNames(classes.pausedTag, isListView ? classes.listPausedTag : '')}>
              Deposits Paused
            </div>
          )}
        </section>
        <Typography noWrap gutterBottom variant="h8" className={classes.title} component="h5">
          {pool.get('name')}
        </Typography>
        <div
          className={classNames(
        ...
        >
          <Typography component="p" className={classes.shopDetailsDesc}>
            <span className={classes.shopDetailsValue}>
              {isFetchVaultsDataPending ? (
                <LinearProgress className={classes.valueLoadBar} />
              ) : (
                pool.get('deposited')
              )}
            </span>
          ...
          <Typography component="p" className={classes.shopDetailsDesc}>
            <span className={classes.shopDetailsValue}>
              {formatTvl(pool.get('tvl'), pool.get('oraclePrice'))}
            </span>
        ...
        {isListView && (
          ...
          >
            ...
            <div className={classes.flexColumn}>
              ...
              <span>
                CRUDE APR: <b>{formatApy(rewardApy || 0)}( {calRewardAPYDaily()} Daily)</b>
              </span>
              <span>
                Total APY: <b>{calTotalApy()}</b>
              </span>
            </div>
            <div className={classes.flexColumn}>
      ...
      <CardActions
        className={classNames(classes.actionRow, showAdditionalInfo ? '' : classes.mt25)}
      >
        <div
          className={classNames(isListView ? classes.shopDetailsBtnCol : classes.shopDetailsBtnRow)}
        >
          <Button
            color="secondary"
            variant="contained"
          ...
          <Button
            color="primary"
            variant="contained"
            className={classes.shopDetailsBtnWithdraw}
            onClick={openDetailModal}
          >
      ...
      <PoolDetailPopup
        ...
        index={index}
        apy={calAPY()}
        apyDaily={calAPYDaily()}
        rewardApy={calRewardAPY()}
        rewardApyDaily={calRewardAPYDaily()}
        totalApy={calTotalApy()}
      />
    </Card>
  );
};
