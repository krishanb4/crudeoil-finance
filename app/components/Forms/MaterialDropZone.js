...
class MaterialDropZone extends React.Component {
  ...
  onDrop(filesVal) {
    ...
    if (oldFiles.length > filesLimit) {
      ...
      //this.props.onCorrectFileSubmitted(oldFiles);
    }
  }
  ...
  render() {
    ...
    const previews = filesArray => filesArray.map((file, index) => {
     ...
     const base64Img = file.isUploaded ? file.path : URL.createObjectURL(file);
      if (isImage(file)) {
        return (
          <div key={index.toString()}>
            <div className="imageContainer col fileIconImg">
              {file.isUploaded && <img className="smallPreviewImg" src={base64Img} alt="preview" />}
             {!file.isUploaded && <figure className="imgWrap"><img className="smallPreviewImg" src={base64Img} alt="preview" /></figure>} 
