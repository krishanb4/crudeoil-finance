...
import { CheckboxRedux, SelectRedux, SwitchRedux } from 'dan-components/Forms/ReduxFormMUI';
...
const ShopAddUpdate = ({
  ...
  const onFormSubmit = values => {
    ...
    if (files && files.length > 0) {
      afterDataValidate(values, files[0]);
    }
  ...
  return (
    <Dialog fullScreen open={open} onClose={handleClose} TransitionComponent={Transition}>
      ...
      <PapperBlock title="Add New Shop" icon="ios-images-outline" whiteBg desc="">
          ...
          <form onSubmit={handleSubmit(onFormSubmit)}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={4}>
                <UploadInputImg
                  onCorrectFileSubmitted={filesSubmitted}
                  ref={com => (fileUploader = com)}
                />
              </Grid>
...
const reducer = 'shop';
const AddUpdateInit = connect(state => ({
  force: state,
  initialValues: state.getIn([reducer, 'shop']),
}))(ShopAddUpdateRedux);

export default AddUpdateInit;
